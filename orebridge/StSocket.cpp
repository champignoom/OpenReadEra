...
#define SERVER_SOCKET_PATTERN "rdr.%08xd.%08xd"
...
{
    LDD(LOG, "StSocket: base: %d %p %d", id, this, socket_fd);
}

StSocketBase::StSocketBase(StSocketBase&& other)
    : socket_fd(other.socket_fd), id(cid++)
{
    LDD(LOG, "StSocket: moved: %d %p <- %d %p: %d", id, this, other.id, &other, socket_fd);
    other.cleanup();
}

void StSocketBase::cleanup()
{
    LDD(LOG, "StSocket: cleanup(): %d %p %d", id, this, socket_fd);
    socket_fd = nullsocket;
}

void StSocketBase::close()
{
    LDD(LOG, "StSocket: close(): %d %p %d", id, this, socket_fd);
    if (isValid())
    {
        ::close(socket_fd);
    }
...
bool StSocketConnection::sendFileDescriptor(int fd)
{
    ...
    *(int*) CMSG_DATA(cmsg) = fd;
    ...
    LDD(LOG, "StSocket: before sendmsg() : %d %d %d", id, socket_fd, fd);
    if (sendmsg(socket_fd, &msg, 0) == -1)
    {
        LE("sendmsg() failed: %d %d", id, errno);
        return false;
    }

    LDD(LOG, "StSocket: sendmsg() sent: %d", fd);
    return true;
}
...
bool StSocketConnection::receiveFileDescriptor(int& fd)
{
    ...
    if (cm->cmsg_type != SCM_RIGHTS)
    {
        ...
        LE("recvmsg() - unknown type: %d", cm->cmsg_type);
        return false;
    }
    ...
    fd = *(int*) CMSG_DATA(cm);
    LDD(LOG, "StSocket: received fd: %d", fd);
    return true;
}
