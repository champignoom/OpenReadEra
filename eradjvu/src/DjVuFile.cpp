...
{
#ifdef OREDEBUG
    //LV("DjVuFile::static_decode_func GO");
#endif
...
#ifdef OREDEBUG
    //LV("DjVuFile::static_decode_func OK");
#endif
}
...
DjVuFile::decode_func(void)
{
  ...
  G_TRY {
    ...
    decode(gpstr);
#ifdef OREDEBUG
    //LV("DjVuFile::decode_func wait_for_finish GO");
#endif
    // Wait for all child files to finish
    while(wait_for_finish(0))
    	continue;
#ifdef OREDEBUG
    //LV("DjVuFile::decode_func wait_for_finish OK");
#endif
  ...
  bool djvi, bool djvu, bool iw44)
{
...
#ifdef OREDEBUG
    //LV("DjVuFile::decode_chunk [%s]", (const char*) chkid);
#endif
  ...
  else if (chkid == "INCL" && (djvi || djvu || iw44))
  {
      ...
      if (file) {
        ...
        desc.format( ERR_MSG("DjVuFile.indir_chunk1") "\t%s", (const char*) file->get_url().fname() );
      } else {
#ifdef OREDEBUG
          //LW("DjVuFile::decode_chunk INCL !file");
#endif
...
#ifdef OREDEBUG
  //LD("DjVuFile::decode_chunk OK [%s][%s]", (const char*) chkid, (const char*) desc);
#endif
  // Return description
  return desc;
}
...
DjVuFile::resume_decode(const bool sync)
{
  bool retval=false;
  {
    GMonitorLock lock(&flags);
#ifdef OREDEBUG
    //LV("DjVuFile::resume_decode %d %d %d, res %d",
    //        is_decoding(), is_decode_ok(), is_decode_failed(),
    //       (!is_decoding() && !is_decode_ok() && !is_decode_failed()));
#endif
