...
#if 0
    CrDom * encDetectionDoc = LVParseHTMLStream( stream );
    int encoding = 0;
...
class CHMTOCReader {
    ...
    bool _fakeToc;
public:
    CHMTOCReader( LVContainerRef cont, CrDom * doc, LvDocFragmentWriter * appender )
...
//            }
        ...
        if ( hhcName.empty() ) {
            ...
            CrDom * doc = LVParseCHMHTMLStream( tocStream, defEncodingName );
            if ( !doc ) {
...
bool ImportCHMDocument(LVStreamRef stream, CrDom* doc, bool cfg_firstpage_thumb_)
{
    ...
    int fragmentCount = 0;
    LvDomAutocloseWriter writer(doc, false, HTML_AUTOCLOSE_TABLE);
