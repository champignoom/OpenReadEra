...
bool HyphMan::activateDictionaryFromStream(LVStreamRef stream)
{
    ...
    if (!method->load(stream)) {
		CRLog::error("HyphMan::activateDictionaryFromStream: error" );
        delete method;
        return false;
    }
    ...
    if (HyphMan::_dictList->find(lString16(HYPH_DICT_ID_DICTIONARY)) == NULL) {
        HyphDictionary * dict = new HyphDictionary(HDT_DICT_ALAN,
        		cs16("Dictionary"), lString16(HYPH_DICT_ID_DICTIONARY), lString16::empty_str);
...
bool HyphDictionary::activate()
{
	...
	if (getType() == HDT_ALGORITHM) {
		CRLog::trace("Turn on algorythmic hyphenation");
