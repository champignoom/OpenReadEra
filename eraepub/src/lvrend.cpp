    ...
    int renderCells( LVRendPageContext & context )
    {
        ...
        for (i=0; i<rows.length(); i++) {
            ...
            for (j=0; j<rows[i]->cells.length(); j++) {
                ...
                if ( i==y ) {
                    ...
                    {
                        int h = renderBlockElement(context, cell->elem, cell->col->x, 0, cell->width);
                        cell->height = h;
        ...
        if ( context.getPageList() != NULL ) {
            ...
            if ( caption && caption_h ) {
                ...
                context.AddLine(y0, y1, line_flags);
            }
...
/*
bool isFullwidthSpace(lChar16 ch)
{
    return (ch == 0x3000 || ch == 0x2001 || ch == 0x2003);
}
*/
...
// Render final block
//=======================================================================
void renderFinalBlock( ldomNode * enode, LFormattedText * txform, RenderRectAccessor * fmt, int & baseflags, int indent, int line_h )
{
    if ( enode->isElement() )
    {
        ...
        if ((flags & LTEXT_FLAG_NEWLINE) && rm != erm_inline)
        {
            ...
            if(enode->getCrDom()->cfg_txt_indent_margin_override && enode->getNodeId() == el_p)
            {
                indent = enode->getCrDom()->cfg_txt_indent;
            }
            else
            {
                switch (len.type)
                {
                    case css_val_percent:
                        indent = width * len.value / 100;
                        break;
                    case css_val_px:
                        indent = len.value;
                        break;
                    case css_val_em:
                        indent = len.value * enode->getFont()->getSize() / 256;
                        break;
                    default:
                        indent = 0;
                        break;
            ...
            //LE("indent for enode [%s] = %d",LCSTR(enode->getNodeName()),indent);
...
//        }
        if ( ntype && ntype->is_object )
        {
            ...
            logfile << "+OBJECT ";
#endif
            //LE("object node [%s]",LCSTR(enode->getXPath()));
            ...
            if ( isBlock ) {
                ...
                txform->AddSourceObject(flags, line_h, indent, enode );
                ...
                txform->AddSourceObject(baseflags, line_h, indent, enode );
        ...
        else
        {
            ...
            for (int i=0; i<cnt; i++)
            {
                ...
                renderFinalBlock( child, txform, fmt, flags, indent, line_h );
            }
            if ( thisIsRunIn ) {
    ...
    else if ( enode->isText() )
    {
      ...
      for (int i=0; i<enode->getNodeLevel(); i++)
        logfile << " . ";
            ...
            if (parent->getNodeName() == "ruby" )
            {
                tflags |= LTEXT_LINE_HAS_RUBY;
            }
            ...
            */
            ...
            if ( txt.length()>0 )
                txform->AddSourceLine( txt.c_str(), txt.length(), cl, bgcl, font, baseflags | tflags,
                    line_h, indent, enode, 0, letter_spacing );
            baseflags &= ~LTEXT_FLAG_NEWLINE; // clear newline flag
        }
    }
    else
...
//            x = x;
        ...
        else if(enode->getCrDom()->cfg_txt_indent_margin_override && (node_id == el_p ))
        {
            margin_bottom  = enode->getCrDom()->cfg_txt_margin;
        }
                   ...
                   ldomMarkedRangeList *bookmarks, lvRect margins,int columns)
{
    if ( !enode->isElement() )
    {
        ...
        int rendMethod = enode->getRendMethod();
        if (rendMethod != erm_table_row && rendMethod != erm_table_row_group)
        {
        ...
        case erm_table_caption:
            {
                ...
                fmt.push();
                {
                    ...
                    {
                        int left_offset = gTextLeftShift;
                        if(!gFlgFloatingPunctuationEnabled || enode->getNodeName() == "image" || enode->getNodeName() == "img")
                        {
                            left_offset = 0;
                        }
                        txform->Draw(&drawbuf, doc_x + x0 + padding_left + left_offset, doc_y + y0 + padding_top, marks, nbookmarks);
                    }
                    if (nbookmarks)
                        delete nbookmarks;
                }
...
void ApplyInlineStyle(ldomNode * enode, css_style_rec_t* pstyle)
{
    ...
    if (gEmbeddedStylesLVL > 0 && enode->hasAttribute(LXML_NS_ANY, attr_style)) {
        ...
        if (!nodeStyle.empty())
        {
            ...
            nodeStyle = Style.getStyleStringFonts();
