...
#include <stdio.h>
#include <string.h>
#include <eraepub/include/crconfig.h>
#include <ore_log.h>
...
class ImageRotateDecoderCallback : public LVImageSource, public LVImageDecoderCallback
{
    //only rotates 90 degrees CCW now
    LVImageSourceRef _src;
    LVImageDecoderCallback* _callback = NULL;
    int w = 0;
    int h = 0;
    int bufsize = 0;
    lUInt32 *buf;
public:
    ...
    explicit ImageRotateDecoderCallback(LVImageSourceRef src)
    {
        _src = src;
        ...
        h = _src->GetHeight();
        bufsize = w * h;
        //LE("w = %d, h = %d, bufsize = %d",w,h,bufsize);
        buf = (lUInt32 *)calloc(bufsize,sizeof(lUInt32));
    }

    // inherited from LVImageDecoderCallback
    ~ImageRotateDecoderCallback() override
    {
        if (buf)
        {
            //LE("free buf");
    ...
    void OnStartDecode( LVImageSource * obj ) override{}
    bool OnLineDecoded( LVImageSource * obj, int y, lUInt32 * data ) override
    {
        //LE("attempt to write %d values to line %d ",w,y);
        memcpy(&buf[y * w], data, w*sizeof(lUInt32) );
        return true;
    }
    void OnEndDecode( LVImageSource * obj, bool errors ) override
    {
        //perform rotation of the whole buffer

        //LE("OnEndDecode START");
        lUInt32 * tempbuf = (lUInt32 *)calloc(bufsize,sizeof(lUInt32));

        if(!tempbuf)
        {
            LE("OnEndDecode Failed allocating %d bytes for operation",bufsize*sizeof(lUInt32));
            return;
        }
        int ptr = 0;
        ...
        for (int x = w-1; x >=0; x--)
        {
            for (int y = 0; y < h; y++)
            {
                tempbuf[ptr++] = buf[(y*w)+x];
            }
        }

        memcpy(buf,tempbuf,bufsize*sizeof(lUInt32));
        free(tempbuf);

        //LE("OnEndDecode END");
    }

    // inherited from LVImageSource
    ldomNode * GetSourceNode() {return _src->GetSourceNode();};
    LVStream * GetSourceStream() {return _src->GetSourceStream();};
    void   Compact() {};
    ...
    {
        int w_temp = GetWidth();
        int h_temp = GetHeight();
        if ( callback )
        {
            int a = w_temp/10;
            callback->OnStartDecode(this);
            //lUInt32 * row = new lUInt32[ w_temp ];
            lUInt32 * row = (lUInt32 *)calloc(bufsize,sizeof(lUInt32));
            for (int y = 0; y < h_temp; y++)
            {
                memcpy(row,&buf[y * w_temp],w_temp * sizeof(lUInt32));
...
void LVGrayDrawBuf::Draw( LVImageSourceRef img, int x, int y, int width, int height, bool dither )
{
    ...
    {
        ImageRotateDecoderCallback rotcb(img);
        img->Decode(&rotcb);
        rotcb.Decode(new LVImageScaledDrawCallback(this, (LVImageSourceRef) &rotcb, x, y, width, height, dither));
    }
}
