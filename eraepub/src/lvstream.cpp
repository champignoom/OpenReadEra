...
//    }
    ...
    static LVFileStream* CreateFileStream(int fd, lvopen_mode_t mode) {
        LVFileStream* f = new LVFileStream;
        if (f->OpenFile(fd, mode) == LVERR_OK) {
            return f;
        } else {
            delete f;
            return NULL;
    ...
    lverror_t OpenFile(int fd, int mode) {
        CRLog::trace("OpenFile via fd %d",fd);
        mode = mode & LVOM_MASK;
        m_fd = fd;
        struct stat stat;
        if (fstat(m_fd, &stat)) {
            CRLog::error("Cannot get file size for fd %d", fd);
            return LVERR_FAIL;
        }
        m_mode = (lvopen_mode_t)mode;
        m_size = (lvsize_t) stat.st_size;
        lString16 name = lString16("/proc/self/fd/");
        name.append(fd);
        SetName(name.c_str());
        return LVERR_OK;
    }
...
// facility functions
LVStreamRef LVOpenFileStream(int fd, int mode)
{
    LVFileStream* stream = LVFileStream::CreateFileStream(fd, (lvopen_mode_t) mode);
    if (stream != NULL) {
