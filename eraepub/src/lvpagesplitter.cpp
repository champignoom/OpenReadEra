...
public:
    ...
    void AddLine( LVRendLineInfo * line )
    {
        ...
        else 
        {
            ...
            //LE("after = %d, before = %d = %d",last->getSplitAfter(), line->getSplitBefore(),flgSplit);
...
//                }
    ...
    void AddFootnoteFragmentToList()
    {
        ...
        if ( footend==NULL )
            footend = footstart;
        //CRLog::trace("AddFootnoteFragmentToList(%d, %d)", footstart->getStart(), footend->getEnd() );
...
void LVRendPageContext::split()
{
    ...
    std::map<int, int> fnotes_filter_map;
        ...
        if ( line->getLinks() ) {
           ...
           // }
            for ( int j=0; j<line->getLinks()->length(); j++ ) {
                ...
                int fnote_id = note->getId().getHash();
                if (fnotes_filter_map.find(fnote_id) != fnotes_filter_map.end())
                {
                    continue;
                }
                fnotes_filter_map[fnote_id] = 1;
