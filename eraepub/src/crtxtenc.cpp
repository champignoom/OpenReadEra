...
bool detectXmlHtmlEncoding(const unsigned char * buf, int buf_len, char * html_enc_name)
{
    ...
    if (xml_p >= 0 && xml_end_p > xml_p) {
        ...
        CRLog::trace("XML header encoding detected: %s", html_enc_name);
        return true;
    }
    ...
    if (content_type_p >= 0) {
        ...
        CRLog::trace("HTML header meta encoding detected: %s", html_enc_name);
        return true;
    }
    return false;
}
...
//	   double q_ = q_1 * q_2;
       ...
       q = q / (q1 + q2);
       //CRLog::trace("%d %10s %4s : %lf %lf %lf - %lf %lf %lf  :  %lf", i, cp_stat_table[i].cp_name, cp_stat_table[i].lang_name, q1, q11, q12, q2, q21, q22, q);
       if (q > bestq) {
		   bestn = i;
		   bestq = q;
	   }
