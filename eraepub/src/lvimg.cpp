...
class WebpImageSource : public LVNodeImageSource
{
public:
    WebpImageSource( ldomNode * node, LVStreamRef stream )
            : LVNodeImageSource(node, stream)
    {
        //LW("creating WebpImageSource for node [%s]",LCSTR(node->getXPath()));
    }

    virtual ~WebpImageSource() {}
    virtual bool Decode( LVImageDecoderCallback * callback )
    {
        if (_stream.isNull())
        {
        ...
        lvsize_t inSize = _stream->GetSize();

        if (inSize < 32 || inSize > 0x80000)
        {
            return false; // wrong size
        }

        auto inBuf = (unsigned char*) malloc(inSize);

        lvsize_t bytesRead = 0;

        _stream->SetPos(0);

        if (_stream->Read(inBuf, inSize, &bytesRead) != LVERR_OK || bytesRead != inSize)
        {
            return false;
        }
        ...
        {
            LE("Webp decode: Unable to get webp dimensions");
            return false;
        }
        _width = w;
        _height = h;
        ...
        const int RGB_NUM = 3;
        const int size = w * h * RGB_NUM;

        auto page_buf = (unsigned char*) malloc(size);
        if (!page_buf)
        {
            LE("Webp decode: failed allocating %d bytes for buf",size);
            return false;
        }
        ...
        page_buf = WebPDecodeRGB(inBuf, inSize, &w, &h);
        if(page_buf == NULL)
        {
            callback->OnEndDecode(this,true);
            free(page_buf);
        ...
        for (int y = 0; y < h; y++)
        {
            for (int x = 0; x < w; x++)
            {
                row[x] = ((lUInt32) page_buf[ptr++] << 16 | (lUInt32) page_buf[ptr++] << 8 | (lUInt32) page_buf[ptr++]);
            }
            if(callback)
            {
                callback->OnLineDecoded(this, y, row);
    ...
    static bool CheckPattern( const lUInt8 * buf, int )
    {
        //check for WEBP signature at beginning of file
        const char webp_start[4] = {0x52,0x49,0x46,0x46};
        const char webp_end[4]   = {0x57,0x45,0x42,0x50};

        if(strncmp((char*)buf,webp_start,4) == 0)
        {
            if(strncmp((char*)buf+8,webp_end,4) == 0)
                return true;
        }
        return false;
    }
...
#endif
#if (USE_WEBP==1)
    if ( WebpImageSource::CheckPattern( hdr, (lUInt32)bytesRead ) )
        img = new WebpImageSource( node, stream );
    else
#endif
