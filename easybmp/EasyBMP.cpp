...
static bool g_exceptions = false;
...
//ORE: wrap for lightweight width/height info header query
bool BMP::GetBmpInfoFromStream(const unsigned char *buffer, size_t size,int *w, int *h)
{
    ReadBuf streambuffer((char*) buffer, size);
    istream stream(&streambuffer);
    return GetBmpInfo_light(stream,w,h);
}

//ORE: lightweight width/height info header query
bool BMP::GetBmpInfo_light(istream& in, int *w, int *h)
{
...
//ORE: dumping pixels to ARGB buffer
bool BMP::WriteToBuffer(unsigned char *buffer, size_t size)
{
    const int RGBA_NUM = 4;
    if(Width * Height * RGBA_NUM > size)
    {
        return false;
    }
    for (int y = 0; y <= this->Height; y++)
    {
        ...
        {
            memcpy(buffer, &Pixels[x][y], RGBA_NUM);
            buffer += RGBA_NUM;
        }
    }
    return true;
