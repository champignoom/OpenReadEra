...
{
	if (fz_is_empty_rect(a))    return 0;
	if (fz_is_empty_rect(b))    return 0;
	if (fz_is_infinite_rect(a)) return 1;
	if (fz_is_infinite_rect(b)) return 0;
...
//returns 1 if a contains point b
int fz_contains_point(fz_rect * a, fz_point p)
{
	if (fz_is_empty_rect(a))    return 0;
	if (fz_is_infinite_rect(a)) return 1;
...
int rects_intersect(fz_rect a, fz_rect b)
{
	fz_rect result;
	...
	return (int)(result.y0 < result.y1 && result.x0 < result.x1);
}

//a includes b
int rect_includes_rect(fz_rect a, fz_rect b)
{
	if(fz_is_empty_rect(&a))
		return 0;
	if (fz_is_infinite_rect(&b))
...
fz_rect union_rect(fz_rect a, const fz_rect b)
{
	fz_rect res;

	res.x0 = min(a.x0,b.x0);
	res.x1 = max(a.x1,b.x1);
	res.y0 = min(a.y0,b.y0);
