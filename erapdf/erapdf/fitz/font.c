...
fz_keep_freetype(fz_context *ctx)
{
	...
	// EraPDF: switch off Adobe darkening >>>
	FT_Bool no_stem_darkening = 1;
	FT_Property_Set( fct->ftlib, "cff", "no-stem-darkening", &no_stem_darkening );
	// EraPDF: switch off Adobe darkening <<<
...
fz_new_font_from_file(fz_context *ctx, const char *name, const char *path, int index, int use_glyph_bbox)
{
	...
	font->ft_face = face;
	// EraPDF: get font type >>>
	font->ft_type = strdup(FT_Get_Face_Type(face));
	// EraPDF: get font type <<<
...
fz_bound_t3_glyph(fz_context *ctx, fz_font *font, int gid, fz_rect *bounds)
{
	...
	fz_try(ctx)
	{
		fz_run_display_list(ctx, list, dev, &font->t3matrix, &fz_infinite_rect, NULL, 0);
	}
	fz_always(ctx)
	{
		fz_drop_device(ctx, dev);
	...
	//  EraPDF: IMHO there is a bug with some matrix: bounds and big uses totally different scales
	//	big = font->bbox;
...
fz_run_t3_glyph(fz_context *ctx, fz_font *font, int gid, const fz_matrix *trm, fz_device *dev)
{
	...
	fz_run_display_list(ctx, list, dev, &ctm, &fz_infinite_rect, NULL, 0);
}
