...
void
fz_invert_tint_pixmap(fz_context *ctx, fz_pixmap *pix)
{
	if (pix->colorspace != fz_device_rgb(ctx))
	{
		fz_throw(ctx, FZ_ERROR_GENERIC, "can only convert RGB cs pixmap");
		return;
	}
    ...
    const char fr = (char)(ctx->f_font_color[0] * 255);
    const char fg = (char)(ctx->f_font_color[1] * 255);
    const char fb = (char)(ctx->f_font_color[2] * 255);

    const char br = (char)(ctx->f_bg_color[0] * 255);
    const char bg = (char)(ctx->f_bg_color[1] * 255);
    const char bb = (char)(ctx->f_bg_color[2] * 255);
	...
	for (x = 0; x < pix->w; x++)
	{
		for (y = 0; y < pix->h; y++)
		{
			mid = 0xFF - ((s[0]+s[1]+s[2]) / 3);
			if(mid > threshold)
			{
				s[0] = fz_mul255(mid, fr);
				s[1] = fz_mul255(mid, fg);
				s[2] = fz_mul255(mid, fb);
			}
			else
			{
				s[0] = br;
...
int fz_count_pixmap_needs_invert(fz_context *ctx, fz_pixmap *pix)
{
	...
	const int GRAY_THRESHOLD = 8;
	int pixnum = 0;
	int alphanum = 0;
	unsigned int coloredpix = 0;

	unsigned int size = pix->w/2 * pix->h/2 * sizeof(int);
    int* coloredArray = (int*)malloc(size);
    if (coloredArray == NULL)
    {
    	LE("fz_count_pixmap_needs_invert ERROR: failed to allocate %d bytes for coloredArray",size);
	    return 0;
    }
    memset(coloredArray,0,size);
	...
	for (int y = 0; y < pix->h; y += 2)
	{
		for (int x = 0; x < pix->w; x += 2)
		{
			...
			int arrindex = ((y/2) * (pix->w/2)) + (x/2);

			if(curralpha < ALPHA_THRESHOLD)
			{
				alphanum++;
                coloredArray[arrindex] = 0;
                continue;
			}
			else if(colored > 0)
			{
				coloredpix++;
                coloredArray[arrindex] = 1;
                continue;
			}
			else
            {
	...
	//LW("pixnum = %d",pixnum);
	//LW("black = %d, white = %d, alpha = %d",black,white,alphanum);
	//LW("b+w+a = %d, threshold = %d",black + white + alphanum,pixnum / 100 * 80 );
	//LW("colored pix count = %d ( = %f )",coloredpix, ((float)coloredpix/(float)pixnum)*100.0F);
	int percent = (ctx->previewmode) ? 4 : 2;

    if((coloredpix > (pixnum/100) * percent)  &&  (coloredpix < (pixnum/100) * (percent + 8)))
    {
    	//LW("Clarifying colored pixels number precisely");
        int lt, t, rt, l, curr, r, lb, b, rb;
        coloredpix = 0;
        const int sub_h = pix->h / 2;
        const int sub_w = pix->w / 2;
        for (int y = 0; y < sub_h; y++)
        {
            for (int x = 0; x < sub_w; x++)
            {
                curr = coloredArray[(y * (pix->w / 2)) + (x)];
                if(curr <= 0 )
                {
	                continue;
                }
                lt = (x == 0 || y == 0)                 ? -1 : coloredArray[((y - 1) * sub_w) + (x - 1)];
                t  = (y == 0)                           ? -1 : coloredArray[((y - 1) * sub_w) + (x)    ];
                rt = (x == sub_w - 1 || y == 0 )        ? -1 : coloredArray[((y - 1) * sub_w) + (x + 1)];

                l =  (x == 0)                           ? -1 : coloredArray[(y * sub_w)       + (x - 1)];
                r =  (x == sub_w - 1)                   ? -1 : coloredArray[(y * sub_w)       + (x + 1)];

                lb = (y == sub_h - 1 || x == 0)         ? -1 : coloredArray[((y + 1) * sub_w) + (x)    ];
                b  = (y == sub_h - 1)                   ? -1 : coloredArray[((y + 1) * sub_w) + (x)    ];
                rb = (y == sub_h - 1 || x == sub_w - 1) ? -1 : coloredArray[((y + 1) * sub_w) + (x + 1)];

                if (lt + t + rt + l + curr + r + lb + b + rb > 0)
                {
                    coloredpix++;
                }
            }
        }
        //LW("NEW colored pix count: %d = ( %f )",coloredpix, ((float)coloredpix / (float) pixnum)*100.0F);
    }
    free(coloredArray);
    ...
    if(ctx->erapdf_twilight_mode && alphanum > (pixnum / 100) * 80)
    {
        return -1;
    }
    return 0;
