...
{
	fz_file_stream *state = stm->state;
	/// EraPDF: corrupted files with garbage on start >>>
	if (whence == SEEK_SET) {
		offset += ctx->erapdf_file_stream_offset;
	}
	/// EraPDF: corrupted files with garbage on start <<<
	...
	/// EraPDF: corrupted files with garbage on start >>>
	stm->pos = n - ctx->erapdf_file_stream_offset;
	/// EraPDF: corrupted files with garbage on start <<<
	stm->rp = state->buffer;
	stm->wp = state->buffer;
}
...
// EraPDF: meta implementation for file >>>
static int meta_file(fz_context *ctx, fz_stream *stm, int key, int size, void *ptr)
{
	fz_file_stream *ps = (fz_file_stream *)stm->state;
	switch(key)
	{
	...
	case FZ_STREAM_META_LENGTH:
		{
			int pos = lseek(ps->file, 0, SEEK_CUR);
			int length = lseek(ps->file, 0, SEEK_END);
			lseek(ps->file, pos, SEEK_SET);
			return length;
		}
	}
	return -1;
