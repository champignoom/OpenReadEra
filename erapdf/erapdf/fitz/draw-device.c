...
fz_draw_fill_image(fz_context *ctx, fz_device *devp, fz_image *image, const fz_matrix *ctm, float alpha, int invert)
{
	...
	fz_try(ctx)
	{
	    ...
	    // EraPDF: twilight mode >>>
        else if(ctx->erapdf_twilight_mode)
        {
	        if (invert == 1)
	        {
		        inverted = fz_new_pixmap(ctx, fz_device_rgb(ctx), pixmap->w, pixmap->h);

		        if(pixmap->colorspace != fz_device_rgb(ctx))
		        {
			        fz_convert_pixmap(ctx,inverted,pixmap);
		        }
		        else
		        {
			        memcpy(inverted->samples, pixmap->samples, pixmap->n * pixmap->w * pixmap->h);
		        ...
		        fz_invert_tint_pixmap(ctx, inverted);
