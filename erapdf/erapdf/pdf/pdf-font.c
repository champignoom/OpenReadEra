...
pdf_load_builtin_font(fz_context *ctx, pdf_font_desc *fontdesc, char *fontname, int has_descriptor)
{
	...
	if (!fontdesc->font)
	{
		...
		// EraPDF: load font from file >>>
		fontdesc->font = fz_new_font_from_file(ctx, fontname, data, 0, 1);
		fontdesc->external_origin = PDF_FD0_BUILTIN;
		// EraPDF: load font from file <<<
	}
...
pdf_load_substitute_font(fz_context *ctx, pdf_font_desc *fontdesc, char *fontname, int mono, int serif, int bold, int italic)
{
	...
	if (!fontdesc->font)
	{
		...
		// EraPDF: load font from file >>>
		fontdesc->font = fz_new_font_from_file(ctx, fontname, data, 0, 1);
		fontdesc->external_origin = PDF_FD0_EXTERNAL;
		// EraPDF: load font from file <<<
...
pdf_load_substitute_cjk_font(fz_context *ctx, pdf_font_desc *fontdesc, char *fontname, int ros, int serif)
{
	...
	if (!fontdesc->font)
	{
		unsigned char *data;
		unsigned int len;
		// EraPDF: load font from file  - index not used >>>
		// int index;

		data = pdf_lookup_substitute_cjk_font(ctx, ros, serif, fontdesc->wmode, &len, NULL);
		// EraPDF: load font from file  - index not used <<<
		if (!data)
...
pdf_load_system_font(fz_context *ctx, pdf_font_desc *fontdesc, char *fontname, char *collection)
{
	...
	data = pdf_lookup_system_font(ctx, fontname, &len);
	if (data) {
		fontdesc->font = fz_new_font_from_file(ctx, fontname, data, len, 0);
	    fontdesc->external_origin = PDF_FD0_SYSTEM;
		return;
	}
	// EraPDF: load system font <<<
...
// EraPDF: check font type >>>
	...
	fz_try(ctx)
	{
		...
		// EraPDF: check font type >>>
		if (fontdesc->font && fontdesc->font->ft_type)
		{
			char* subtype = fontdesc->font->ft_type;
			if (!strcmp(subtype, "truetype") || !strcmp(subtype, "cff"))
				kind = TRUETYPE;
			else
				kind = TYPE1;
		}
		...
		{
			// EraPDF: check font type <<<
			pdf_obj *subtype = subtype = pdf_dict_get(ctx, dict, PDF_NAME_Subtype);
