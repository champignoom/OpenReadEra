...
{
	char buf[20];
	/// EraPDF: files with garbage in the beginning >>>
	int i;

	for (i = 0; i < 1024; i++)
	{
		ctx->erapdf_file_stream_offset = i;
		...
		if (memcmp(buf, "%PDF-", 5) == 0)
		{
			...
			if (i > 0)
			{
				fz_warn(ctx, "Possible stream offset %d", i);
			}
			return;
		}
	}
	fz_throw(ctx, FZ_ERROR_GENERIC, "cannot recognize version marker");
	ctx->erapdf_file_stream_offset = 0;
	/// EraPDF: files with garbage in the beginning <<<
}
...
// EraPDF: First try to find last %%EOF marker >>>
	int found_eof = 0;
	do
	{
		...
		if (!found_eof) {
			doc->file_size -= (int)sizeof buf;
		}
	} while (!found_eof && (t != 0));

	if (!found_eof) {
		fz_warn(ctx, "EOF marker not found");
...
pdf_xref_size_from_old_trailer(fz_context *ctx, pdf_document *doc, pdf_lexbuf *buf)
{
	...
	int ofs;
	pdf_obj *trailer = NULL;
// EraPDF: Fix for some broken xrefs >>>
	int k;
// EraPDF: Fix for some broken xrefs <<<
...
// EraPDF: Fix for some broken xrefs >>>
        ...
        if (!(c >= '0' && c <= '9') && c != 't')
        {
            ...
            for (k = 0; k < len; k++)
            {
                fz_seek(ctx, doc->file, 18, SEEK_CUR);
                if (EOF == pdf_lex_skip_whites(ctx, doc->file))
                {
                    break;
                }
            }
...
// EraPDF: Fix for some broken xrefs >>>
			n = fz_read(ctx, file, (unsigned char *) buf->scratch, 18);
			if (n != 18)
...
pdf_init_document(fz_context *ctx, pdf_document *doc)
{
	...
	fz_try(ctx)
	{
// EraPDF: proper reparing >>>
		ctx->erapdf_reparing = 0;
// EraPDF: proper reparing <<<

		pdf_load_version(ctx, doc);
// EraPDF: files with garbage at beginning >>>
		doc->file_length = fz_stream_meta(ctx, doc->file, FZ_STREAM_META_LENGTH, 0, NULL) - ctx->erapdf_file_stream_offset;
// EraPDF: files with garbage at beginning <<<
...
// EraPDF: We work only with files, So always try to load progressive >>>
//		if (fz_stream_meta(ctx, doc->file, FZ_STREAM_META_PROGRESSIVE, 0, NULL) > 0)
			doc->file_reading_linearly = (ctx->erapdf_linearized_load && !ctx->erapdf_has_password) ? 1 : 0;
// EraPDF: We work only with files, So always try to load progressive <<<
...
object_updated:
	...
	{
	    LE("cannot find object in xref (%d %d R) - not loaded yet?", num, gen);
