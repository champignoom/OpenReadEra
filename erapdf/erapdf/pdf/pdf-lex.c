...
// EraPDF: Check for %%EOF comment>>>
static int
lex_eof_marker(fz_context *ctx, fz_stream *f)
{
    static int marker[]  = { '%', 'E', 'O', 'F' };
    int index;
    ...
    int last = fz_read_byte(ctx, f);
    return last == '\012' || last == '\015' || last == EOF;
}
// EraPDF: Check for %%EOF comment <<<
...
// EraPDF: wrap lex_whites function >>>
int pdf_lex_skip_whites(fz_context *ctx, fz_stream *f)
{
    lex_white(ctx, f);
    return fz_peek_byte(ctx, f);
}
// EraPDF: wrap lex_whites function <<<
...
pdf_lex(fz_context *ctx, fz_stream *f, pdf_lexbuf *buf)
{
	while (1)
	{
		...
		case '%':
// EraPDF: Check for %%EOF comment >>>
			if (lex_eof_marker(ctx, f) && !ctx->erapdf_reparing)
			{
				return PDF_TOK_EOF;
			}
// EraPDF: Check for %%EOF comment <<<
