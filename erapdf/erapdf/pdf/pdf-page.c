...
// EraPDF: Fix for strange documents with /Auto keys instead of /Type >>>
static pdf_obj *
pdf_get_node_type(fz_context *ctx, pdf_obj *node)
{
    pdf_obj *type_obj = pdf_dict_get(ctx, node, PDF_NAME_Type);
    if (!type_obj)
    {
        type_obj = pdf_dict_get(ctx, node, PDF_NAME_Auto);
    }
    return type_obj;
}
// EraPDF: Fix for strange documents with /Auto keys instead of /Type <<<
...
// EraPDF: Fix for strange documents with /Auto keys instead of /Type >>>
				pdf_obj *type = pdf_get_node_type(ctx, kid);
// EraPDF: Fix for strange documents with /Auto keys instead of /Type <<<
	...
	fz_catch(ctx)
	{
	// EraPDF: Ignore most errors >>>
		if (!ctx->erapdf_ignore_most_errors)
			fz_rethrow(ctx);
	// EraPDF: Ignore most errors <<<
	}
...
pdf_count_pages_before_kid(fz_context *ctx, pdf_document *doc, pdf_obj *parent, int kid_num)
{
	...
	for (i = 0; i < len; i++)
	{
		...
		if (pdf_to_num(ctx, kid) == kid_num)
			return total;
// EraPDF: Fix for strange documents with /Auto keys instead of /Type >>>
		if (pdf_name_eq(ctx, pdf_get_node_type(ctx, kid), PDF_NAME_Pages))
// EraPDF: Fix for strange documents with /Auto keys instead of /Type <<<
		{
...
// EraPDF: Fix for strange documents with /Auto keys instead of /Type >>>
	if (!pdf_name_eq(ctx, pdf_get_node_type(ctx, node), PDF_NAME_Page) != 0)
// EraPDF: Fix for strange documents with /Auto keys instead of /Type <<<
...
pdf_load_page(fz_context *ctx, pdf_document *doc, int number)
{
	...
	if (doc->file_reading_linearly)
	{
		...
		if (pageref == NULL)
		// EraPDF: Fix for files that only declares itself as "Linearized" >>>
		{
			// Try to load non-progressive
